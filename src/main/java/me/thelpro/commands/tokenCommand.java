...
public class tokenCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f[&eTokens&f] /token help &7for help."));
            return true;
        }
        if (args[0].equalsIgnoreCase("help")) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', "-----------------------&f[&eTokens&f]-----------------------\n/token import (i) &7to make a item into a token.\n&f/token submit (s) &7to submit a token.\n&f/token info &7to view info about plugin.\n&f-----------------------------------------------------"));
            return true;
        }

        else if (args[0].equalsIgnoreCase("import") || args[0].equalsIgnoreCase("i")) {
            if (!player.hasPermission("token.import")) {
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f[&eTokens&f] &cYou can not preform this command!"));
                return true;
            } else if (player.hasPermission("token.import")) {
            ...
            if (!player.hasPermission("token.submit")) {
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f[&eTokens&f] &cYou can not preform this command!"));
                return true;
            } else if (player.hasPermission("token.submit")) {
                ...
                if (newTokens == 0) {
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f[&eTokens&f] &cThere were no tokens for you to submit!"));
                    return true;
                }
